...
public class InformationManager : MonoBehaviour
{
    ...
    public float GetDistanceTo(Vector3 crewmate){
        float deltaX = player.position.x - crewmate.x;
        float deltaY = player.position.y - crewmate.y;
        float deltaZ = player.position.z - crewmate.z;
        return (float)Math.Round(new Vector3(deltaX, deltaY, deltaZ).magnitude,0);
    }
